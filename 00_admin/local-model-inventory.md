...
- 该 OM 已通过 ONNX checker 和 ATC 编译，输入源为 `1x3x640x640`，输出为 `1x84x8400`。当前 runner 源码已支持 descriptor 选择 RGB_PLANAR FP32/NV12 路径，但尚未在 SS928 上读取该 OM 的真实 descriptor 或完成检测对齐，因此仍不能标记为直接部署。
...
| `08_media/models/ss928_yolo11n/yolo11n_ss928.om` | `9e3c448...16af95` | SS928 候选 | ATC 与 runner 源码契约通过；许可、descriptor 和检测结果待实板/授权 |
...
当前仍需实板完成：ACL descriptor 检查、RGB_PLANAR 实际输入验证、同图 OM 与已通过的 PT/ONNX 参考检测对齐、双 USB 快照、长期稳定性和温度资源测试；正式发布前还需解决模型和仓库根许可。
