...
`smartbag-vision.service` 是单侧诊断 unit，不属于 `smartbag.target`。`smartbag-alternating-vision.service` 是默认关闭的交替实验入口，与正式 alert/video 服务互斥。`smartbag-cloud-uploader.service` 也不属于默认 target，云端故障不会阻塞本地告警。IMX347 MIPI preview/VO 只保留在 firmware samples，不加入默认启动链。
