...
打开 `http://<板端地址>:8081/`；顶部“低延迟交替画面”始终显示左右两侧中最新的一帧，下方两个窗口每秒读取一次各侧最近缓存快照和帧龄。页面默认只维持顶部一条连续 MJPEG，避免三条重复视频流争用网络和浏览器解码。`stream_only` 不生成检测 overlay，首页会在左右 overlay 不可用时自动选择 raw，并禁用按钮显示“检测画面不可用”。原始 session 在 `/var/log/smartbag/alternating-camera-runs/<SESSION_ID>/`，包括 `session.json`、`switch-events.csv`、`camera-events.csv`、`performance.csv`、`alerts.csv`、`errors.log` 和 summary。
...
若重点是低延迟调试，可再降到 `640x480`，把 `--stream-fps-limit` 提高到 30，并优先观察顶部交替流。本机两只 UVC 摄像头已验证 `SLICE_MS=300 WARMUP_FRAMES=0 FRAMES_PER_SLICE=4`：80 帧连续解码无黑帧或损坏帧，左右各约 4.85 FPS，顶部交替流约 8.52 FPS，帧间隔中位数约 40 ms、p95 约 311 ms、最大约 340 ms。`alternating-test.sh` 使用这组纯预览默认值；若更换摄像头后首帧异常，可显式设置 `WARMUP_FRAMES=1`。单侧流天然会在另一侧采集时冻结；这不是以太网带宽不足，也不能通过回放旧帧真正修复。两路相机共用同一 USB 2.0 控制器时，每次 STREAMOFF/STREAMON 的首帧等待仍是主要抖动来源。
...
2026-07-20 实测从异常千兆切到 100M 全双工后，20 张 JPEG 吞吐由约 0.7 Mbps 升至 27.3 Mbps；取消已确认无必要的单帧预热丢弃后，顶部交替流由约 7.63 FPS 提升到约 8.52 FPS，最长帧间隔由约 400 ms 降到约 340 ms。该结果说明现场千兆 PHY/网线兼容异常会叠加视频卡顿，但 100M 已足够当前 MJPEG 调试流。不要仅凭小包 ping 判断视频链路正常。
