...
try:
    ...
    print(config.get("alternating_camera", {}).get("detector_backend", "ultralytics"))
...
try:
    config = json.load(open(sys.argv[1], encoding="utf-8"))
    print(config.get("alternating_camera", {}).get(
        "ss928_runtime_library",
        "/root/smartbag/vision/ss928_backend/lib/libsmartbag_ss928_acl.so",
    ))
except (OSError, ValueError):
    print("/root/smartbag/vision/ss928_backend/lib/libsmartbag_ss928_acl.so")
PY
)
...
"$PYTHON" - "$DETECTOR_BACKEND" <<'PY' || fail=1
import importlib
import platform
import sys
...
backend = sys.argv[1]
missing = []
required = ["cv2", "numpy", "dbus", "gi"]
if backend == "ultralytics":
    required.extend(["torch", "torchvision", "ultralytics", "lap"])
...
if [ "$DETECTOR_BACKEND" = ss928_om ]; then
    [ -f "$SS928_ADAPTER" ] \
        && echo "OK   SmartBag SS928 ACL adapter $SS928_ADAPTER" \
        || { echo "MISS SmartBag SS928 ACL adapter $SS928_ADAPTER" >&2; fail=1; }
fi
