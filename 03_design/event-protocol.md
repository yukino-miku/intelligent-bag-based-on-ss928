...
Controller 将 detector 标记为 `vision:left|right|auto` source；可选 MR20 使用 `radar:<name>`。内部状态键是 `(source, side)`，因此某来源的 level=0 只清该来源。取同侧最大有效等级后驱动 TM6605/灯/legacy PWM，并通过统一 BLE TX 推送：
...
跌倒/撞击事件使用独立类型 `fall_confirmed`、`impact_only`、`possible_fall`，不得直接转换为交通震动等级。累计驼背输出 `REMINDER,HUNCH,...`，Controller 将其转为独立 `posture:hunch` 双侧轻提醒并按配置超时，不清除视觉/雷达状态。GNSS 和姿态数据也使用独立 `type`。
