...
2. 在板端确认 Ubuntu/aarch64、Python、BlueZ、两路 UVC、USB 拓扑、I2C0/TCA、UART4、TM6605、PWM 灯、I2S、可选 MR20/MT5710/Tsensor 和存储空间。
...
5. 运行 `install.sh <仓库根目录>`，生成并迁移 `/etc/smartbag/config.json`、root-only env 和 systemd units；首次安装可用 `SMARTBAG_HARDWARE_PROFILE` 套用 profile。
...
8. 真实硬件验收必须覆盖 detector/来源隔离、safe-off、固定左右路由、双摄持续帧率/温度、TCA channel、TM6605/灯、GNSS checksum、BMI/驼背/跌倒、可选 MR20/5G/音频、BLE 唯一服务、手机真机和 reboot 自启。
