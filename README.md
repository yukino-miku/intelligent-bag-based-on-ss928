...
- **雷达主导融合**：双 MR20 持续输出完整 RadarScan；每个目标拥有独立 generation 轨迹、车型绑定和风险稳定状态，不会先压缩成单一最高目标。
- **交替快照分类**：左右 UVC 任意时刻最多一路 STREAMON，共用一个 YOLO/OM 模型；不运行视觉测距、测速、BoT-SORT、光流或视觉风险。
- **目标级风险**：雷达点按真实标定投影到图像，与 bbox 一对一全局匹配；车型绑定后只在原 RiskModel 内乘一次权重，无视觉时使用 unknown=1.0 继续报警。
- **告警输出**：Controller 使用每条雷达轨迹多帧稳定后的 haptic_level，按侧选最危险目标，驱动 TCA9548A 后的左右 TM6605/LRA、Pin7/Pin32 灯和可选 MAX98357 音频。
...
默认示例 profile 是 `radar_primary_visual_classification`：双 MR20 + 单模型交替快照 + BMI270 + TM6605/LRA + 灯。Controller 是 BLE、振动、灯、音频、雷达 worker 和融合运行时的唯一所有者；GNSS/BMI 默认 `--no-ble`。摄像头或 YOLO 失败不会清空雷达风险，只会退化为 unknown 类别；服务停止、事件过期和异常仍由 safe-off 清除输出。

## NPU/OM 当前状态

`ss928_backend` 现已增加持久 runner 服务模式，模型只加载一次；交替分类器把最新 BGR 快照转成 NV12 后请求 OM 推理。代码不会伪造检测结果，但仓库没有合法且已验收的车辆 OM。工厂 OM 历史上未输出可信目标，因此 OM 正确率、双 MR20、真实投影、每侧快照频率和 30 分钟稳定性仍是硬件验收项；OpenVINO 仍只代表 CPU 优化，不等于 SS928 NPU。

## 已验证与限制

- Windows 本地已通过 335 项 Python、11 个小程序/CloudBase Node 测试文件、4 个 NPU native C++ tests 和 1 个 C 兼容核心测试，并完成 Python/JS/JSON/Shell 静态检查；命令口径和限制见 [integration-status](00_admin/integration-status.md)。
