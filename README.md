...
- SS928：`board_cpu` 基线、稳定 haptic `vision_alert` JSONL、单摄左右方向推断、双摄兼容、四路 PWM 震动、可选 MAX98357 音频。
- 传感器：DX-GP21 GNSS/NMEA/轨迹；BMI270 IIO/I2C、姿态和短时运动趋势；独立跌倒/撞击事件。
- 移动端：统一 `SS928-SmartBag` BLE NUS，支持告警、GNSS、IMU 和系统命令；原生微信小程序显示轨迹和姿态。
...
默认只启动单个 detector；双摄需显式配置并先验证算力。GNSS 与 BMI270 默认 `--no-ble`，由 controller 独占 BLE，避免多个 NUS 冲突。震动只使用稳定后的 `haptic_risk_level`，detector 退出、事件过期、异常或服务停止都会清零 PWM。

## 未完成与安全边界

- `Ss928OmBackend` 尚无真实 MPP/SVP/NPU API，只定义接口；OpenVINO CPU 不是 SS928 NPU。
