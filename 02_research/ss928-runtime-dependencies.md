...
| NumPy | 未安装 | APT 候选 1.21.5 |
| OpenCV | 未安装 | APT 候选 4.5.4 |
...
1. 先运行 `install-board-cpu-deps.sh` 安装系统 OpenCV、NumPy、v4l2 工具和基础服务。
...
4. 安装后必须运行 `check-runtime-deps.sh`；任何 import 失败都不能记为板端 YOLO 已通过。
