...
当前两台 UVC 摄像头位于同一个 USB 2.0 控制器和 Hub 下，同时 `STREAMON` 会触发 `ENOSPC`。原生 V4L2 交替 `STREAMON/OFF` 已在 SS928 完成 1800.247 秒纯采集，3620/3620 次切换成功且无 `ENOSPC`。实测 capture-only 最大盲区 580.264 ms；带模型时仍必须另测完整端到端观测间隔。该方案只适合作为资源受限比赛原型，不等同于同步双摄，也不适合作为最终高安全等级产品方案。
...
- 当前 500 ms 时间片下，实测 capture-only 最大单侧盲区约 0.51 至 0.54 秒。
- `end_to_end_observation_gap_ms` 按同一侧两张真正进入算法的帧的 capture timestamp 计算。因为单主循环会在下一次采集前完成解码、推理、跟踪、风险、overlay 和 JPEG，它自然包含完整处理和另一侧轮回；最终阈值必须使用 E2E max。
- 每片默认仍采集 warmup 2 + valid 4，但只选择最后 1 张最新有效帧推理。其余帧记录后立即跳过，不建立待处理队列，防止旧观测延迟数秒后触发风险。
- 单目测距、跟踪和风险状态必须按左右完全隔离。
- CAUTION 以上普通确认必须跨不同 slice；同一 burst 的多张相近帧不能直接满足 DANGER。fast path 仅限极近、高质量冲突并记录原因。
- tracker buffer 可按真实每侧有效 FPS 调整，速度、CPA 和风险仍只依赖 monotonic timestamp。
...
- 一侧断线只关闭和重开该侧；另一侧继续。长时间断线后仅重置该侧 tracker。自动化 mock 已覆盖，真实左右拔插/PWM 清振仍需板端实测。
