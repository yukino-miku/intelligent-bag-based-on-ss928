...
该桥接仅为“车辆类别快照分类”服务，不承担视觉测距、测速、跟踪或风险判断。正式融合模式由 MR20 连续提供运动数据，OM 结果只为当前图片关联写入 `bicycle/motorcycle/car/truck/bus` 类别映射；无视觉结果、关联歧义或映射过期时仍以 `unknown=1.0` 运行共享 RiskModel，不保留持久车型绑定。

本地审计已找到一个 YOLO11n 候选 OM，但其输入是 `RGB_PLANAR` 静态 AIPP，现有 runner 固定输入为 NV12/YUV420SP，二者不兼容。正式路径统一为 `/root/smartbag/models/vehicle-detector.om`；在适配 runner 并完成 ACL descriptor 与同图识别对齐之前，模型部署状态保持 `BLOCKED`，不能仅凭 ATC 编译成功宣称可用。
